...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, return_hidden=False, mask=None, text_vec=None):
        # print(text)
        ...
        inf = torch.zeros((x.size(1), x.size(1))).fill_(float("-inf")).repeat(x.size(0), 1, 1).to(mask.device)
        mask = mask.unsqueeze(1).expand(-1, mask.size(1), -1) # [4, 18, 18]
        attn_mask = torch.where(mask>0, attn_mask, inf) # [4, 18, 18]
        # print(attn_mask,attn_mask.shape, mask, mask.shape,"====")
